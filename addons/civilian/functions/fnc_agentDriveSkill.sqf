/*
	...
	Author:
	RCA3 (https://forums.bohemia.net/forums/topic/223430-ai-driving-control/)
...
if (!(_car isKindOf "Car" || _car isKindOf "Motorcycle" || _car isKindOf "Tank") || {isPlayer _driver} || {_car getVariable "NOAIDRIVINGCONTROL"} || {!(local _driver)}) exitWith{};
	...
	if (isNull _stopcar) then{
		...
		if (_carsarray isNotEqualTo []) then{
			...
			if (_carsinfrontarray isNotEqualTo []) then{
                ...
                _carsinfrontsorted = _carsinfrontarray apply {[_carfrontpos distance _x, _x]};
	...
	if (_scan && {!((behaviour _driver) isEqualTo "STEALTH")}) then{ //Scan and bypass scan on STEALTH
		switch (true) do{
			...
			case (_frontcar isKindOf "Man"):{
                ...
                // Check if _speed is not NaN
                if ((typeName _speed) isEqualTo "SCALAR") then {
                    AIDCCARSPEED(_speed);
                };
			};
