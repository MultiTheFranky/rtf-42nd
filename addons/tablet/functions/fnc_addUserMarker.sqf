...
/*
	...
	Example:
		...
		["bluefor",[[1714.35,5716.82],0,0,0,"12:00",player]]call FUNC(addUserMarker);
		
		// Client receiving marker addition (from server)
		["bluefor",[21,[[1714.35,5716.82],0,0,0,"12:00",player]],157]call FUNC(addUserMarker);
*/
...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			[[_encryptionKey,[_transactionId,_markerData],_transactionId],"FUNC(addUserMarker)",true,false,true] call bis_fnc_MP;
	...
	if (hasInterface) then {
		[_this,"FUNC(addUserMarker)",false,false,true] call bis_fnc_MP;
	};
};
