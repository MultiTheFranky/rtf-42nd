...
/*
	...
	Example:
		[str _uavVehicle,[[0,"rendertarget8"],[1,"rendertarget9"]]] call FUNC(createUavCam);
*/
...
_uavCams = _this select 1;
{
	...
	if ((_camPosMemPt != "") && (_camDirMemPt != "")) then {
		...
		private _index = cTabUAVcams pushBack [_uav,_renderTarget,_cam,_camPosMemPt,_camDirMemPt];
	};
} count _uavCams;
