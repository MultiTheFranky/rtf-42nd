...
/*
	...
	Example:
		...
		["bluefor",5] call FUNC(deleteUserMarker);
		
		// Client receiving request for marker deletion (from server)
		["bluefor",5,158] call FUNC(deleteUserMarker);
*/
...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			if (_removeIndex != -1) then {
				...
				[[_encryptionKey,_markerIndex,cTab_userMarkerTransactionId],"FUNC(deleteUserMarker)",true,false,true] call bis_fnc_MP;
	...
	if (hasInterface) then {
		[_this,"FUNC(deleteUserMarker)",false,false,true] call bis_fnc_MP;
	};
};
