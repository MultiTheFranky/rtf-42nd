...
if (_mode != 2) then {
	// ------------------ VEHICLES ------------------
	{
		...
		private _index = _vehicles pushBack _veh;
				...
				if (_drawText) then {
					...
					if (_vehIndex == -1 || {(groupID group _veh) != _text}) then {
						...
						if (_mountedIndex != -1) then {
							...
							private _index = _mountedLabels pushBack _veh;
							private _index = _mountedLabels pushBack _text;
						};
					};
				};
			};
		...
		if (_mode != 2 && {_veh == _playerVehicle || {_veh in _vehicles}}) exitWith {
			if (_drawText) then {
				...
				if (_mountedIndex != -1) then {
					...
					private _index = _mountedLabels pushBack _veh;
					private _index2 = _mountedLabels pushBack (_x select 4);
				};
			};
		};
