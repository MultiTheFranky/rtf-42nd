...
/*
	...
	Example:
		...
		[player] call FUNC(getUserMarkerList);
		
		// Client receiving the list from the server
		[[userMarkerList],157] call FUNC(getUserMarkerList);
*/
...
call {
	...
	if (count _this == 0) exitWith {
		if (hasInterface && !isServer) then {
			[[player],"FUNC(getUserMarkerList)",false,false,true] call bis_fnc_MP;
		};
	};
	...
	if (count _this == 1) exitWith {
		if (isServer && !hasInterface && (cTab_userMarkerTransactionId >= 0)) then {
			[[cTab_userMarkerLists,cTab_userMarkerTransactionId],"FUNC(getUserMarkerList)",_this select 0,false,true] call bis_fnc_MP;
		};
	};
