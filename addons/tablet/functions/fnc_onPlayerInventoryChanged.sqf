...
/*
	Name: FUNC(onPlayerInventoryChanged)
	...
	Example:
		[ACE_player,[player] call ace_common_fnc_getAllGear] call FUNC(onPlayerInventoryChanged);
*/
...
_playerLostDevice = call {
	if (_displayName == "cTab_Tablet_dlg") exitWith {
		!("rtf42_tablet_Tablet" in _itemsToCheck)
	};
