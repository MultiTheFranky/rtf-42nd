...
_classNames = [];
{
	if (isClass (configfile >> "CfgVehicles" >> _x) && _classNames find _x == -1) then {
		private _index = _classNames pushBack _x;
	};
...
// iterate through all class names and add child classes, so we end up with a list of helmet classes that have the defined helmet classes as parents
{
	_childClasses = "inheritsFrom _x == (configfile >> 'CfgWeapons' >> '" + _x + "')" configClasses (configfile >> "CfgWeapons");
	{
		...
		if (_classNames find _childClassName == -1) then {
			private _index = _classNames pushBack configName _x;
		};
	} count _childClasses;
} forEach _classNames;
...
FUNC(onIfMainPressed) = {
	...
	_interfaceName = call {
		...
		if ([_player,["ItemMicroDAGR"]] call FUNC(checkGear)) exitWith {
			cTabMicroDAGRmode = [2, 0] select ([_player,["rtf42_tablet_Tablet"]] call rtf42_tablet_fnc_checkGear);
			"cTab_microDAGR_dsp"
		};
		if ([_player,_vehicle,"FBCB2"] call FUNC(unitInEnabledVehicleSeat)) exitWith {"cTab_FBCB2_dlg"};
		if ([_player,["rtf42_tablet_Tablet"]] call FUNC(checkGear)) exitWith {"cTab_Tablet_dlg"};
		// default
		""
	};
...
cTabOnDrawbftTAD = {
	...
	_cntrlScreen drawIcon [QPATHTOF(ui\TAD_overlay_ca.paa),cTabTADfontColour,_playerPos,250,250,_heading,"",1,cTabTxtSize,"TahomaB","right"];
...
[] spawn {
	...
	["cTab","ifMain",["Toggle Main Interface","Open cTab device in small overlay mode if available"],{call FUNC(onIfMainPressed)},"",[cTab_key_if_main_scancode,cTab_key_if_main_modifiers],false] call cba_fnc_addKeybind;
	["cTab","ifSecondary",["Toggle Secondary Interface","Open cTab device in interactable mode"],{call FUNC(onIfSecondaryPressed)},"",[cTab_key_if_secondary_scancode,cTab_key_if_secondary_modifiers],false] call cba_fnc_addKeybind;
	["cTab","ifTertiary",["Toggle Tertiary Interface","Open private cTab device when in a vehicle with its own cTab device, or to open Tablet while also carrying a MicroDAGR"],{call FUNC(onIfTertiaryPressed)},"",[cTab_key_if_tertiary_scancode,cTab_key_if_tertiary_modifiers],false] call cba_fnc_addKeybind;
	["cTab","zoomIn",["Zoom In","Zoom In on map while cTab is in small overlay mode"],{call FUNC(onZoomInPressed)},"",[cTab_key_zoom_in_scancode,cTab_key_zoom_in_modifiers],false] call cba_fnc_addKeybind;
	["cTab","zoomOut",["Zoom Out","Zoom Out on map while cTab is in small overlay mode"],{call FUNC(onZoomOutPressed)},"",[cTab_key_zoom_out_scancode,cTab_key_zoom_out_modifiers],false] call cba_fnc_addKeybind;
...
cTab_msg_gui_load = {
	...
	// Messages
	{
		...
		_img = call {
			if (_msgState == 0) exitWith {QPATHTOF(ui\icoUnopenedmail.paa)};
			if (_msgState == 1) exitWith {QPATHTOF(ui\icoOpenmail.paa)};
			if (_msgState == 2) exitWith {QPATHTOF(ui\icon_sentMail_ca.paa)};
		};
	...
	{
		if ((side _x in _validSides) && {isPlayer _x} && {[_x,["rtf42_tablet_Tablet","ItemAndroid"]] call FUNC(checkGear)}) then {
...
cTab_msg_Send = {
	...
	if (_recipientNames != "") then {
		_msgArray = cTab_player getVariable [format ["cTab_messages_%1",_playerEncryptionKey],[]];
		private _index = _msgArray pushBack [format ["%1 - %2",_time,_recipientNames],_msgBody,2];
		cTab_player setVariable [format ["cTab_messages_%1",_playerEncryptionKey],_msgArray];
...
["cTab_msg_receive",
	{
		...
		private _index = _msgArray pushBack [_msgTitle,_msgBody,0];
		
		_msgRecipient setVariable [format ["cTab_messages_%1",_msgEncryptionKey],_msgArray];
		
		if (_msgRecipient == cTab_player && _sender != cTab_player && {_playerEncryptionKey == _msgEncryptionKey} && {[cTab_player,["rtf42_tablet_Tablet","ItemAndroid"]] call FUNC(checkGear)}) then {
