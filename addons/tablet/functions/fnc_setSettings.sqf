...
/*
	...
	Parameters:
 	...
 	Example:
		["cTab_Tablet_dlg",[["mapType","SAT"],["mapScaleDsp","4"]]] call FUNC(setSettings);
		
		// Update mapWorldPos and update the interface even if the value has not changed
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player]],true,true] call FUNC(setSettings);
		
		// Update mapWorldPos and mapScale, but do not update the interface
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player],["mapScaleDsp","2"]],false] call FUNC(setSettings);
*/
