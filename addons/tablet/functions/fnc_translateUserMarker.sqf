...
/*
	...
	Example:
		[[1714.35,5716.82],0,0,0,"12:00"] call FUNC(translateUserMarker);
*/
...
_texture1 = call {
	...
	if (_markerIcon == 7) exitWith {QPATHTOF(ui\o_inf_rifle.paa)};
	if (_markerIcon == 8) exitWith {QPATHTOF(ui\o_inf_mg.paa)};
	if (_markerIcon == 9) exitWith {QPATHTOF(ui\o_inf_at.paa)};
	if (_markerIcon == 10) exitWith {QPATHTOF(ui\o_inf_mmg.paa)};
	if (_markerIcon == 11) exitWith {QPATHTOF(ui\o_inf_mat.paa)};
	if (_markerIcon == 12) exitWith {QPATHTOF(ui\o_inf_mmortar.paa)};
	if (_markerIcon == 13) exitWith {QPATHTOF(ui\o_inf_aa.paa)};
...
_align = ["right", "left"] select ((_dir > 0) && (_dir < 180));
