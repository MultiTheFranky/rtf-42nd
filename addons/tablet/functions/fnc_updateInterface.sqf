...
/*
	...
	(Optional)
 	...
 	Example:
		[[["mapType","SAT"],["mapScaleDsp","4"]]] call FUNC(updateInterface);
*/
...

{
	call {
		...
		if (_x select 0 == "nightMode") exitWith {
			...
			_nightMode = [false, true] select ((_nightMode == 1) || {_nightMode == 2 && (sunOrMoon < 0.2)});
			_background = call {
				if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) exitWith {
					[QPATHTOF(ui\TAD_background_ca.paa), QPATHTOF(ui\TAD_background_night_ca.paa)] select (_nightMode);
				};
				if (_displayName in ["cTab_Android_dsp","cTab_Android_dlg"]) exitWith {
					[QPATHTOF(ui\android_background_ca.paa), QPATHTOF(ui\android_background_night_ca.paa)] select (_nightMode);
				};
				if (_displayName in ["cTab_microDAGR_dsp","cTab_microDAGR_dlg"]) exitWith {
					[QPATHTOF(ui\microDAGR_background_ca.paa), QPATHTOF(ui\microDAGR_background_night_ca.paa)] select (_nightMode);
				};
				if (_displayName in ["cTab_Tablet_dlg"]) exitWith {
					[QPATHTOF(ui\tablet_background_ca.paa), QPATHTOF(ui\tablet_background_night_ca.paa)] select (_nightMode);
				};
				""
			};
			if (_background != "") then {
