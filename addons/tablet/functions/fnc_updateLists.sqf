...
*/
{
	if ((_x != cTab_player) && {[_x,["rtf42_tablet_Tablet","ItemAndroid","ItemMicroDAGR"]] call FUNC(checkGear)}) then {
		private _index = _cTabBFTmembers pushBack [_x,_x call FUNC(getInfMarkerIcon),"",name _x,str([_x] call CBA_fnc_getGroupIndex)];
	};
...
*/
{
	if ((side _x in _validSides) && {_x != _playerGroup}) then {
		_leader = objNull;
		call {
			if ([leader _x,["rtf42_tablet_Tablet","ItemAndroid"]] call FUNC(checkGear)) exitWith {_leader = leader _x;};
			{
				if ([_x,["rtf42_tablet_Tablet","ItemAndroid"]] call FUNC(checkGear)) exitWith {_leader = _x;};
			} count units _x;
		};
		if !(IsNull _leader) then {
			...
			private _index = _cTabBFTgroups pushBack [_leader,"\A3\ui_f\data\map\markers\nato\b_inf.paa",_sizeIcon,groupID _x,""];
		};
	};
} count allGroups;
...
*/
{
	if ((side _x in _validSides) && {count (crew _x) > 0} && {_x != _playerVehicle}) then {
		...
		_iconB = "";
		call {
			if (_x isKindOf "MRAP_01_base_F") exitWith {_iconA = QPATHTOF(ui\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "MRAP_02_base_F") exitWith {_iconA = QPATHTOF(ui\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "MRAP_03_base_F") exitWith {_iconA = QPATHTOF(ui\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "Wheeled_APC_F") exitWith {_iconA = QPATHTOF(ui\b_mech_inf_wheeled.paa);};
		...
		call {
			...
			private _index = _cTabBFTvehicles pushBack [_x,_iconA,_iconB,_name,_groupID];
		};
	};
} count vehicles;
...
*/
{
	if (side _x in _validSides) then {
		private _index = _cTabUAVlist pushBack _x;
	};
} count allUnitsUav;

/*
cTabHcamlist --- HELMET CAMS
Units on our side, that have either helmets that have been specified to include a helmet cam, or rtf42_tablet_TabletHCAM in their inventory.
*/
{
	if (side _x in _validSides) then {
		if (headgear _x in cTab_helmetClass_has_HCam || {[_x,["rtf42_tablet_TabletHCam"]] call FUNC(checkGear)}) then {
			private _index = _cTabHcamlist pushBack _x;
		};
	};
} count allUnits;
...
if !(cTabUAVlist isEqualTo _cTabUAVlist) then {
	...
	private _index = _updateInterface pushBack ["uavListUpdate",true];
};
if !(cTabHcamlist isEqualTo _cTabHcamlist) then {
	cTabHcamlist = [] + _cTabHcamlist;
	private _index = _updateInterface pushBack ["hCamListUpdate",true];
};
