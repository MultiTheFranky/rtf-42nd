...

{
	...
	_handle = _ctrl ctrlSetEventHandler ["LBSelChanged", 
		format ["[_this select 1, '%1',%2,%3,%4] call CHVDAddon_fnc_onLBSelChanged_syncmode", _x select 1, _x select 2, _x select 3, _x select 4]
	];
...

{
	...
	_handle = _ctrlDisplay ctrlSetEventHandler ["keyDown", 
		format ["[_this select 0, '%1',%2,%3] call CHVDAddon_fnc_onEBinput_syncmode", _x select 1, _x select 2, _x select 3]
	];
